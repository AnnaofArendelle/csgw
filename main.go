...
func cmdStart(ctx context.Context, args []string) error {
	...
	// 先把端口绑上再做联网自检：网络慢的时候不至于让人先吃一个 connection refused。
	addr, err := gw.listen()
	if err != nil {
		return err
	}
	...
	fmt.Printf("连接：%s%s%s    （Ctrl-C 停止网关；停机交给 GitHub 的 idle 设置）\n\n",
		cBright, connectHint(cfg.listen()), cReset)

	if !*noVerify {
		changed, err := checkTokenBeforeServing(ctx, cfg)
		if err != nil {
			return err
		}
		if changed {
			// 向导换了 token，provider 手里那个已经过期了。这时候还没开始 accept，
			// 直接替换是安全的。
			if prov, err = newCodespacesProvider(ctx, cfg, logger); err != nil {
				return err
			}
			gw.prov = prov
		}
...
func checkTokenBeforeServing(ctx context.Context, cfg *Config) (changed bool, err error) {
	...
	info, err := newAPI(token).whoamiQuick(checkCtx)
	switch {
	case err == nil:
	...
	case tokenRejected(err):
		...
		if !isTTY() {
			return false, fmt.Errorf("换一个 token 再启动：`%s setup`（或者设置 $GITHUB_TOKEN）", appName)
		}
